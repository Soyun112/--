...
### Tmap (4) — **appKey 하나로 모두 사용**
- `POST https://apis.openapi.sk.com/tmap/routes/pedestrian` — 보행자 길찾기
- `GET https://apis.openapi.sk.com/tmap/pois` — POI 검색 (반경·명칭)
- `GET https://apis.openapi.sk.com/tmap/geo/fullAddrGeo` — 주소 → 좌표
- `POST https://apis.openapi.sk.com/tmap/road/matchToRoads` — 이동한도로찾기 (경로 도로 스냅)

> 이미지의 **TMAP 대중교통·TMAP TMS**는 별도 상품/계약 API입니다. 이 앱은 **TMAP Open API**(보행·POI·지오코딩·Road)를 사용합니다.
