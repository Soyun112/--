...
async function fetchShareById(shareId) {
  const endpoints = [
    `${API_BASE}/api/share/kid-guide/${encodeURIComponent(shareId)}`,
    `${API_BASE}/api/share/${encodeURIComponent(shareId)}`,
  ];
...
async function loadGuide() {
  ...
  const inline = typeof readInlineGuidePayload === "function" ? readInlineGuidePayload() : null;
  if (inline?.steps?.length) {
    showApp(inline);
    return;
  }

  const shareId = typeof readShareIdFromLocation === "function" ? readShareIdFromLocation() : null;
  if (shareId) {
    try {
